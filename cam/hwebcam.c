...
void webcam(bool capture_picture, int english, char *bouton1, char *bouton2)
{
	...
	char button_1[10] = {'\0'}, button_2[10] = {'\0'}, button_3[10] = {'\0'}, button_4[10] = {'\0'};
	int valButtonsUpdated = 0;
...
//#ifdef USE_SDL
	...
	SDL_Surface       *backgroundPicture = NULL;
	SDL_Rect           video_rect;
...
/** Options par defaut **/
	if (!capture_picture) sdl_window = 1;
	else timeout = 1;
	...
	if (sdl_window) {
		...
		/* Initialize the display requesting a software surface */ 
			surface = SDL_SetVideoMode(1280, 720, 24, /*SDL_FULLSCREEN|*/SDL_SWSURFACE|SDL_ANYFORMAT);
			if (surface == NULL) {
			...
			if (english == 0) backgroundPicture = SDL_LoadBMP("./pics/Page_2_Photo_FR.bmp");
			else backgroundPicture = SDL_LoadBMP("./pics/Page_2_Photo_EN.bmp");
			
			if (backgroundPicture == NULL) printf("Error: Picture Loading");
			SDL_BlitSurface(backgroundPicture, NULL, surface, &backgroundPosition);
			SDL_Flip(surface);
	...
	printf("First Video Frame NOT ready\n");
	pollret = poll(pfds, 1, -1);
	printf("First Video Frame ready\n");
	...
	while (keep_going || keep_going_theora || keep_going_vorbis) {
		printf("KEEPGOING\n");
		/* We Read the button states */
		Read (bouton1, button_1);	/* Wait for value 1 on Button 1 */
		Read (bouton2, button_2);
		valButtonsUpdated = updateVal(&button_1[0], &button_2[0], &button_3[0], &button_4[0]);
		printf("ValButton : %d\n", valButtonsUpdated);
		if (valButtonsUpdated == 2) {page_index = 2+english; break;}
		if (valButtonsUpdated == 1) {page_index = 0+english; break;}
	...
	//	if (capture_picture) keep_going = 0;
	}//while() 
	fprintf(stderr, "\n");
